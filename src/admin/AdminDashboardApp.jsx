  ...
  async function approveDeposit(depositId) {
    ...
    try {
      ...
      if (deposit.status === 'approved') {
        showToast('This deposit is already approved.', 'info')
        return
      }
      ...
      if (!balanceData) {
        const { error: insertError } = await supabase
          ...
          .insert({
            user_id: userId,
            etb_wallet: currency === 'ETB' ? depositAmount : 0,
            usd_wallet: currency === 'USD' ? depositAmount : 0,
          })
        if (insertError) throw insertError
      } else {
        const updatePayload = currency === 'USD'
          ? { usd_wallet: (Number(balanceData.usd_wallet) || 0) + depositAmount }
          : { etb_wallet: (Number(balanceData.etb_wallet) || 0) + depositAmount }
      ...
      showToast('Deposit approved and balance updated!', 'success')
      await refresh()
    } catch (err) {
      console.error('Approve error:', err)
      showToast('Failed to approve: ' + (err?.message || String(err)), 'error')
    } finally {
      setLoading(false)
      setBusyId(null)
    }
